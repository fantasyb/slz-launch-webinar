...
- **`cairn:audit-log verify [--against <seq>:<hash>]`** — re-walk the chain; any edit, deletion, reorder, or **tail truncation** (caught via the head sidecar) is detected and the exact line named. `--against` additionally checks the live log still matches an anchor you are holding off the box.
