...
export const keysDir = () => homePath('keys');
...
export function loadKeys(): Map<string, KeyRecord> {
  ...
  if (!fs.existsSync(keysDir())) return (cache = map);

  for (const file of fs.readdirSync(keysDir()).filter((f) => f.endsWith('.json'))) {
    const rec = JSON.parse(fs.readFileSync(path.join(keysDir(), file), 'utf8')) as KeyRecord;
