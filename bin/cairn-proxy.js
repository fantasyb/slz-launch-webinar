#!/usr/bin/env node
/**
 * Launcher for the MCP gateway — findings ride back on results.
 *
...
const bundle = path.join(__dirname, '..', 'dist', 'cli', 'mcp-proxy.js');
if (fs.existsSync(bundle)) {
  require(bundle);
} else {
  const { spawn } = require('child_process');
  spawn('npx', ['tsx', path.join(__dirname, '..', 'scripts', 'mcp-proxy.ts'), ...process.argv.slice(2)], {
    stdio: 'inherit',
