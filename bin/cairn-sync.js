#!/usr/bin/env node
/**
 * Launcher for sync — the agent keeps its own corpus current.
 *
...
const bundle = path.join(__dirname, '..', 'dist', 'cli', 'sync.js');
if (fs.existsSync(bundle)) {
  require(bundle);
} else {
  const { spawn } = require('child_process');
  spawn('npx', ['tsx', path.join(__dirname, '..', 'scripts', 'sync.ts'), ...process.argv.slice(2)], {
    stdio: 'inherit',
