...
/* A fixture token, assembled at runtime so no bearer-credential literal is in
...
/** A home whose org policy requires auth and defines one readonly principal. */
 ...
 */
function hit(base: string, pathname: string, opts: { method?: string; headers?: Record<string, string>; body?: string } = {}): Promise<{ status: number; headers: http.IncomingHttpHeaders; body: string }> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = http.request(
      { hostname: u.hostname, port: u.port, path: u.pathname, method: opts.method ?? 'GET', headers: { ...(opts.headers ?? {}), connection: 'close' } },
      (res) => {
        let body = '';
...
test('a governed gateway rejects an unauthenticated request at the door, audits it, and accepts a valid token', async () => {
  ...
  try {
    ...
    // The refusals were written to the tamper-evident log.
    _resetAuditCache();
    const auditDir = path.join(home, 'audit');
    const entries = readAudit(auditDir);
    assert.ok(entries.length >= 2, `each rejected attempt is one audit row (got ${entries.length})`);
    assert.ok(entries.every((e) => e.decision === 'auth-fail'), 'and every row so far is an auth failure');
